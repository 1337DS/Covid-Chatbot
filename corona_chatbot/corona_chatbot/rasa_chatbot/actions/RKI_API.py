...
class Endpoint_Germany(Endpoint_Requester):
  ...
  def get_history(self, metric, hospitalization=None, days=None, as_df=True):
    ...
    if not metric in self.valid_history_endpoints:
      print(f"You must provide a valid endpoint.       Available endpoints are: {self.valid_history_endpoints}")
      return None, None
    ...
    # extract the correct sub-metric from "hospitalization" metric
      ...
      print(f"You didn't specify a valid option for the parameter 'hospitalization'.       Valid options are: {self.valid_hospitalization_endpoints}")
      return None, None
